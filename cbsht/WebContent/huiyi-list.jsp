		...
		<table class="table table-striped table-hover table-bordered">
			...
			<c:forEach items="${list }" var="hy">
				<tr>
					...
					<td><a
						href="<%=request.getContextPath()%>/huiyi/ry?id=${hy.huiyi_id }&p=${page }">${hy.renyuan.size() }人参会,详情</a></td>
					<td><a
						href="<%=request.getContextPath()%>/huiyi/xq?id=${hy.huiyi_id }&p=${page }">修改</a>、<a
						href="<%=request.getContextPath()%>/huiyi/del?id=${hy.huiyi_id }&p=${page }">删除</a></td>
				</tr>
			</c:forEach>
		</table>
		<form id="ds" action="<%=request.getContextPath()%>/huiyi/list"
			method="post">
			<table>
				<tr>
					<td><a
						href="<%=request.getContextPath() %>/huiyi/list?p=${page-1}&s=${s}">上一页</a></td>
					<td>第<input id="p" type="text" name="p" value="${page}">/${pages}页<input
						type="submit" value="转到"></td>
					<td><a
						href="<%=request.getContextPath() %>/huiyi/list?p=${page+1}&s=${s}">下一页</a></td>
				</tr>
			</table>
		</form>
